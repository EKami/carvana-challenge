...
def center_cropping_resize(img, new_size):
    ...
    """
    lowest = min(img.width, img.height)
    new_h = np.round(np.multiply(new_size[0] / lowest, img.size[0])).astype(int)
    new_w = np.round(np.multiply(new_size[1] / lowest, img.size[1])).astype(int)
...
def get_center_crop_size(img_path, img_size):
    ...
    lowest = min(img.width, img.height)
    new_h = np.round(np.multiply(img_size[0] / lowest, img.size[0])).astype(int)
    new_w = np.round(np.multiply(img_size[1] / lowest, img.size[1])).astype(int)
    return new_h, new_w